...
아래 항목만 상세히 묘사하라: 피부톤과 전반적인 인상에서 자연스럽게 드러나는 특징(예: 동아시아계/서양계/흑인계/히스패닉계 등 캐스팅 참고용 표현), 성별, 추정 연령대, 얼굴 생김새(눈·코·입·얼굴형 등 구체적 특징), 헤어스타일과 헤어 컬러.
이 정보는 동일한 인상의 모델을 다시 캐스팅하기 위한 화보 제작 목적으로만 쓰인다. 최대한 구체적으로 답하되, 확신이 서지 않으면 "~계열로 추정됨" 같은 근사적 표현을 사용해서라도 반드시 답하라.
