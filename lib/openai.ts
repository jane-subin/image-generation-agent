...
이 정보는 동일한 인상의 모델을 다시 캐스팅하기 위한 화보 제작 목적으로만 쓰인다. 인물이 동아시아계 인상이면, **여성일 경우 반드시 "예쁜 한국인 모델"로, 남성일 경우 "예쁜"이라는 표현 없이 "한국인 모델"로** 특정하여 서술하라. 그 외의 인상이면 해당 특징을 명확하고 구체적으로 서술하라.
...
- 외모: 피부톤과 전반적인 인상, 성별, 추정 연령대, 얼굴 생김새(눈·코·입·얼굴형 등 구체적 특징), 헤어스타일과 헤어 컬러. 동아시아계 인상이면, 여성일 경우 반드시 "예쁜 한국인 모델"로, 남성일 경우 "예쁜"이라는 표현 없이 "한국인 모델"로 특정하여 서술하라.
